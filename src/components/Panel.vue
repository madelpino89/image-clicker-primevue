        ...
        <div class="grid">
                                    ...
                                    <div
                                        ...
                                        <div class="flex flex-column align-items-center sm:align-items-start gap-3">
                                            <div class="hidden md:block text-2xl font-bold text-900">
                                                <Button link :label="slotProps.data.name"
                            ...
                            <div class="col-12 sm:col-6 lg:col-12 xl:col-4 p-2">
                                <div class="p-4 border-1 surface-border surface-card border-round">
                                    <div class="flex flex-column align-items-center gap-3 py-5">
                                        <img class="w-9 sm:w-10rem xl:w-10rem shadow-2 border-round"
                                            :src="slotProps.data.picture.large" :alt="slotProps.data.email" />
                                        <div class=" hidden md:block text-2xl font-bold">
                                            <Button link :label="slotProps.data.name"
                                                @click="showUserDetail(slotProps.data)" />
                                        </div>
                                        <div class="block md:hidden text-2xl font-bold text-900">
                                            <Button link :label="slotProps.data.name" @click="showDialog(slotProps.data)" />
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="xl:col-6 lg:col-6 hidden md:block">
                <div v-if="isUserDetailEmpty">
                    <div class="grid grid-nogutter surface-section text-800">
                        <div class="col-12 md:col-6 p-6 text-center md:text-left flex align-items-center ">
                            <section>
                                <span class="block text-6xl font-bold mb-1">Select one person</span>
                                <div class="text-6xl text-primary font-bold mb-3">for see details</div>
                                <p class="mt-0 mb-4 text-700 line-height-3">
                                    Click on any person's name in the list to see their details and the number of times
                                    their profile has been visited.
                                </p>
                            </section>
                        </div>
                    </div>
                ...
                <div v-else class="card">
                    <div v-if="userDetail.picture.large"
                        class="inline-flex align-items-center justify-content-center gap-2">
                        <Avatar size="xlarge" :image="userDetail.picture.large" shape="circle" />
                        <span class="font-bold white-space-nowrap">
                            {{ userDetail.name }}
                        </span>
                        <Button text plain size="small" label="Viewed" icon="pi pi-eye" severity="secondary"
                            :badge="userDetail.clicked" class="cursor-auto" badgeClass="p-badge-success" />
                    </div>
                    <div>
                        <ul class="list-none p-0 mt-4">
                            <li v-if="userDetail.email"
